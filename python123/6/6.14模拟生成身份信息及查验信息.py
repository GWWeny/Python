...
def person_name(gender_of_id, last_name_file, male_name_file, female_name_file):
    ...
    with open(last_name_file, 'r',encoding='utf-8') as f:
        name=[line.strip().replace('，','').replace('。', '') for line in f]
        name1=''.join(name[:51])
        name2=''.join(name[51:])
        name=list(name1)+[name2[i*2:i*2+2] for i in range(len(name2)//2)]
        surnames=random.choice(name)

    with open(male_name_file, 'r',encoding='utf-8') as f:
        male_names=f.readline().split()
    with open(female_name_file, 'r',encoding='utf-8') as f:
        female_names=f.readline().split()

    if gender_of_id=='男':
        names=random.choice(male_names)
    else:
        names=random.choice(female_names)
...
def id_of_17(area_of_code, birth_date, birth_order):
    ...
    """
    area_code = random.choice([key for key in area_of_code.keys() if not key.endswith('00')])
...
def id17_to_18(id_number):
    ...
    return id_number + check_codes[total % 11]
...
def village_of_live(village_file, area_of_code):
    ...
    if(area_id[:2] in ['11','12','31','50']):
        address=f"{province}{area}"
    else:
        address=f"{province}{city}{area}"
    address = address + f"{village}{building}栋{door}单元{floor:02}{room:02}室"
...
def all_of_nation(nation_file):
    ...
    with open(nation_file,'r',encoding='utf-8') as f:
        ...
        # nations=f.readlines()
    return random.choice(nations).strip().replace('族','')
...
def area_of_registration(id_number, area_of_code):
    ...
    county_code=id_number[:6]
    province=area_of_code.get(province_code,"")
    city=area_of_code.get(city_code,"")
    county=area_of_code.get(county_code,"")
    if city:
...
def area_of_live(area_id, area_of_code):
    ...
    其他地区格式例如：湖北省武汉市洪山区。
    """
    province_code=area_id[:2]+"0000"
    city_code=area_id[:4]+"00"
    province=area_of_code.get(province_code,"")
    city=area_of_code.get(city_code,"")
    if province in ['11','12','31','50']:
        county=area_of_code[area_id]
        return f"持证人居住于{province}{county}"
    else:
        county=area_of_code[area_id]

        return f"持证人居住于{province}{city}{county}"
...
def check_city(id_number, city_name, city_code, area_live):
    ...
    返回值均为字符串类型。
    """
    if area_live == city_name:
        return f"持证人居住于{city_name}市"
    elif city_code[:4] in id_number[:4]:
